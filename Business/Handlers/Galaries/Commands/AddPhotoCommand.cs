...
namespace Business.Handlers.Galaries.Commands
{
    ...
    public class AddPhotoCommad : IRequest<IResult>
    {
        ...
        public class AddPhotoCommadHandler : IRequestHandler<AddPhotoCommad, IResult>
        {
            ...
            {
                var result = await _mediator.Send(new GetGalaryQuery { GalaryId = request.GalaryId });
                if (request.File.Length > 0)
                {
                    string folderPath = Path.Combine(_hostingEnvironment.WebRootPath, "uploads/galary");
                    ...
                    result.Data.Photo = "/uploads/galary/" + request.File.FileName;
                    /*myClass.Photo = "/uploads/" + file.FileName; */
                    var upResult = await _mediator.Send(new UpdateGalaryCommand()
                    {
                        GalaryId = result.Data.GalaryId,
